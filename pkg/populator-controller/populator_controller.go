...
import (
	...
	popinformers "github.com/kubernetes-csi/external-provisioner/client/informers/externalversions/volumepopulator/v1alpha1"
	poplisters "github.com/kubernetes-csi/external-provisioner/client/listers/volumepopulator/v1alpha1"
...
func NewPopulatorController(
	...
	eventRecorder = broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: fmt.Sprintf("populator-controller")})
...
func (ctrl *populatorController) Run(workers int, stopCh <-chan struct{}) {
	...
	klog.Infof("Starting populator controller")
	defer klog.Infof("Shutting populator controller")
...
func (ctrl *populatorController) syncPvcByKey(key string) error {
	...
	if !valid {
		ctrl.eventRecorder.Event(pvc, v1.EventTypeWarning, "UnrecognizedDataSourceKind",
			"The data source for this PVC does not match any registered volume populator")
	}
