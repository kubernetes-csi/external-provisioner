...
func (p *csiProvisioner) prepareProvision(ctx context.Context, claim *v1.PersistentVolumeClaim, sc *storagev1.StorageClass, selectedNodeName string) (*prepareProvisionResult, controller.ProvisioningState, error) {
	...
	if p.supportsTopology() {
		...
		snapshotTopologyConstrained := false
		if utilfeature.DefaultFeatureGate.Enabled(features.VolumeSnapshotTopology) &&
			rc.snapshot && isImmediateBinding(sc) && len(snapshotNodeAffinity) > 0 {
			snapshotTopologyConstrained = true
			...
			if len(allowedTopologies) == 0 {
				...
				if p.nodeDeployment != nil && selectedNodeName != "" {
					// Node-deployment uses the selected-node annotation to record
					// ownership even for Immediate binding. Release that ownership
					// when the snapshot topology changed after the node was selected.
					state = controller.ProvisioningReschedule
				}
				return nil, state, fmt.Errorf("no compatible topology: snapshot %s NodeAffinity does not intersect StorageClass %s AllowedTopologies, so the volume cannot be provisioned where the snapshot is accessible", dataSource.Name, sc.Name)
			}
		...
		if apierrors.IsNotFound(err) {
			...
			state := controller.ProvisioningNoChange
			if snapshotTopologyConstrained && p.nodeDeployment != nil && selectedNodeName != "" {
				// The node-deployment owner is no longer compatible with the
				// snapshot. Remove selected-node so another instance can try.
				state = controller.ProvisioningReschedule
			}
			return nil, state, fmt.Errorf("error generating accessibility requirements: %v", err)
		}
...
func (p *csiProvisioner) checkNode(ctx context.Context, claim *v1.PersistentVolumeClaim, sc *storagev1.StorageClass, caller string) (provision bool, err error) {
	...
	case "":
		...
		if !isImmediateBinding(sc) || !p.nodeDeployment.ImmediateBinding {
			return false, nil
		}

		// The StorageClass topology, plus snapshot topology for Immediate
		// snapshot restores, must match this node before it can become owner.
		allowedTopologies := sc.AllowedTopologies
		if p.supportsTopology() &&
			...
			if err != nil {
				if logger.Enabled() {
					logger.Infof("%s: deferring snapshot topology check for PVC %s/%s until provisioning: %v", caller, claim.Namespace, claim.Name, err)
				}
			} else if dataSource != nil &&
				dataSource.Kind == snapshotKind &&
				dataSource.APIVersion == snapshotAPIGroup &&
				p.snapshotClient != nil &&
				p.controllerCapabilities[csi.ControllerServiceCapability_RPC_CREATE_DELETE_SNAPSHOT] {
				_, snapshotNodeAffinity, err := p.getSnapshotSource(ctx, claim, sc, dataSource)
				if err != nil {
					if logger.Enabled() {
						logger.Infof("%s: deferring snapshot topology check for PVC %s/%s until provisioning: %v", caller, claim.Namespace, claim.Name, err)
					}
				} else if len(snapshotNodeAffinity) > 0 {
					allowedTopologies = intersectSnapshotTopology(sc.AllowedTopologies, snapshotNodeAffinity)
					if len(allowedTopologies) == 0 {
						if logger.Enabled() {
							logger.Infof("%s: ignoring PVC %s/%s, snapshot NodeAffinity does not intersect StorageClass AllowedTopologies", caller, claim.Namespace, claim.Name)
						}
						return false, nil
					}
				}
		...
		// Validate this node against the effective allowed topologies. If that
		// fails, another node-deployed provisioner may be compatible.
		if len(allowedTopologies) > 0 && p.supportsTopology() {
